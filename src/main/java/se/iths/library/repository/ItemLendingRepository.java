...
public interface ItemLendingRepository extends CrudRepository<ItemLending, Long> {
    ...
    @Query("SELECT new se.iths.library.dto.BorrowedItemsDTO( i.title, i.barCode, il.creationDate, il.dueDate) FROM ItemLending il INNER JOIN il.item i INNER JOIN il.user u WHERE u.id=:id AND il.isConfirmed = TRUE")
